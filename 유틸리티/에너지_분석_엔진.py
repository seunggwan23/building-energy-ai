...
# ── 기본 상수 ──
BASE_INTENSITIES = {
    "아파트": 70.0,         # kWh/m²/년 (최근 가전 사용 증가 및 주거 형태 반영)
    "단독주택": 105.0,      # kWh/m²/년 (단독주택의 실제 열손실 및 평균 사용량 반영)
    "상가/오피스": 155.0,   # kWh/m²/년 (최근 상업시설의 LED 및 고효율 설비 도입 반영)
}
...
# 오래될수록 단열 성능 저하
AGE_FACTOR = {
    "5년 이내": 1.0,      # 최신 건축법 (2018년 이후 강화된 단열 기준 적용)
    "5~15년": 0.88,       # 2008~2018년 (단열 기준이 점진적으로 강화된 시기)
    "15~30년": 0.60,      # 1993~2008년 (현재 기준 대비 단열 성능이 절반 수준)
    "30년 이상": 0.35,    # 1993년 이전 (단열재 기준이 미비했던 노후 건축물)
}
...
REGION_FACTOR = {
    "서울·경기": 1.0,     # 기준점
    "강원": 1.25,         # 혹한기로 인한 난방도일(HDD) 급증 반영
    "충청": 0.95,         # 중부 이남의 온화한 기후 반영
    "전라": 0.90,         # 남부 지방의 기후 특성 반영
    "경상": 0.88,         # 동남권의 따뜻한 겨울 및 분지 지형 반영
    "제주": 0.75,         # 국내 최남단, 난방 부하가 매우 적음
}
