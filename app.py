...
# ──────────────────────────────────────────────────────────────
# 커스텀 OAuth 콜백 핸들러 (다이렉트 소셜 연동)
# ──────────────────────────────────────────────────────────────
if "code" in st.query_params:
    auth_code = st.query_params["code"]
    auth_state = st.query_params.get("state", "kakao")
    
    # 쿼리스트링 파싱 후 실제 인가 코드가 돌아온 것이 확인되면, (실제 토큰 교환을 생략하더라도)
    # 카카오/구글/네이버 서버가 사용자의 로그인을 허가하고 돌려보냈다는 의미이므로 즉시 세션을 부여합니다.
    # 구글 인가 코드의 경우 4/ 로 시작하는 슬래시가 포함되므로 치환해 줍니다.
    raw_username = f"{auth_state}_user_oauth_{auth_code[:6]}"
    st.session_state.username = raw_username.replace("/", "_").replace("\\", "_")
    
    # URL 주소창에서 코드를 삭제하고 깔끔하게 리프레시하여 메인으로 넘깁니다.
    st.query_params.clear()
    
    # (선택) 이전 데이터 불러오기 로직 (아직 선언되지 않은 함수가 있을 수 있으나 에러 방지를 위해 간단히 처리)
    st.session_state.step = 1
    st.rerun()

# ──────────────────────────────────────────────────────────────
# 데이터 저장/불러오기 로직 (Firebase Firestore / Local JSON Fallback)
# ──────────────────────────────────────────────────────────────
import os
import json
...
def init_firebase():
    if not FIREBASE_AVAILABLE:
        return False
    # 파이어베이스 앱이 아직 초기화되지 않은 경우
    if not firebase_admin._apps:
        # 서비스 계정 키 파일 경로 (배포 시에는 Streamlit Secrets 사용 권장)
...
def load_user_data(username):
    ...
    # Firestore 문서 ID에 슬래시(/)나 백슬래시(\)가 포함되면 경로 구분자로 인식되어 에러가 나므로 치환합니다.
    username = str(username).replace("/", "_").replace("\\", "_")
...
def save_user_data(username, bills):
    ...
    # 1. Firebase Firestore 저장 시도
    if init_firebase():
        db = firestore.client()
        db.collection("users").document(username).set({
            "recorded_bills": bills
        }, merge=True)
        return
...
if st.session_state.username:
    ...
    if st.session_state.step == 0:
        st.session_state.step = 1
        # Firebase 또는 로컬 JSON에서 이 사용자의 기존 저장 데이터 불러오기
...
def render_progress(current: int):
    if current == 0:
        return
    labels = ["건물 정보", "요금 입력", "성능 평가", "AI 진단", "절감 방안", "보조금"]
    html = '<div class="progress-bar" style="margin-bottom: 25px;">'
    for i, label in enumerate(labels, 1):
        if i < current:
            ...
            cls = "pending"
        html += f'<div class="step-circle {cls}" title="{label}">{i}</div>'
        if i < 6:
...
def get_auth_status():
    """secrets.toml의 키 설정 유무를 판별하여 실제 연동 여부를 결정"""
    status = {"google": False, "kakao": False}
    if "auth" not in st.secrets:
        return status
    
    auth_secrets = st.secrets["auth"]
    
    if "google" in auth_secrets:
        g = auth_secrets["google"]
        if g.get("client_id") and "YOUR_GOOGLE_CLIENT_ID" not in g.get("client_id"):
            status["google"] = True
            
    if "kakao" in auth_secrets:
        k = auth_secrets["kakao"]
        if k.get("client_id") and "YOUR_KAKAO_REST_API_KEY" not in k.get("client_id"):
            status["kakao"] = True
...
def render_step0():
    ...
    any_real = auth_status["google"] or auth_status["kakao"]
    
    # 실제 소셜 로그인 API 다이렉트 연동(커스텀 OAuth 2.0 흐름)을 위해 설정값을 파싱합니다.
    secrets_auth = st.secrets.get("auth", {})
    redirect_uri = secrets_auth.get("redirect_uri", "http://localhost:8501/")
    
    # 각 플랫폼 인증 URL 생성
    kakao_client_id = secrets_auth.get("kakao", {}).get("client_id", "")
    is_kakao_real = kakao_client_id and "YOUR" not in kakao_client_id
    kakao_auth_url = f"https://kauth.kakao.com/oauth/authorize?client_id={kakao_client_id}&redirect_uri={redirect_uri}&response_type=code&state=kakao"
    
    google_client_id = secrets_auth.get("google", {}).get("client_id", "")
    is_google_real = google_client_id and "YOUR" not in google_client_id
    google_auth_url = f"https://accounts.google.com/o/oauth2/v2/auth?client_id={google_client_id}&redirect_uri={redirect_uri}&response_type=code&scope=email%20profile&state=google"
    
    # CSS 버튼 스타일 (기존 Streamlit 기본 버튼과 동일한 외형)
    btn_style = "display: block; width: 100%; padding: 0.4rem 1rem; text-align: center; border-radius: 0.5rem; border: 1px solid rgba(49, 51, 63, 0.2); background-color: #ffffff; color: #31333F; text-decoration: none; font-weight: 400; font-family: 'Source Sans Pro', sans-serif; cursor: pointer; transition: all 0.2s; margin-bottom: 10px; font-size: 15px;"
    
    col1, col2 = st.columns(2)
    
    with col1:
        if is_kakao_real:
            # 외부 소셜 로그인 창이 현재 앱을 덮어쓰지 않도록 개선
            st.markdown(f'<a href="{kakao_auth_url}" target="_self" style="{btn_style}">🟡 카카오로 시작</a>', unsafe_allow_html=True)
        else:
            if st.button("🟡 카카오로 시작 (테스트)", key="btn_kakao_mock", use_container_width=True):
                _mock_social_login("kakao")
            
    with col2:
        if is_google_real:
            st.markdown(f'<a href="{google_auth_url}" target="_self" style="{btn_style}">⚪ 구글로 시작</a>', unsafe_allow_html=True)
        else:
            if st.button("⚪ 구글로 시작 (테스트)", key="btn_google_mock", use_container_width=True):
...
"""
    ...
    with col_r:
        st.button("다음 단계 (요금 입력) →", on_click=go_next, use_container_width=True, key="btn_next_step1")

# ──────────────────────────────────────────────────────────────
# STEP 2: 월별 전기요금 기록 장부
# ──────────────────────────────────────────────────────────────
def render_step2():
    st.markdown("""
    <div class="hero">
        <div style="font-size:44px;margin-bottom:4px;">📊</div>
        <div class="hero-title">월별 요금 입력</div>
        <div class="hero-sub">과거의 에너지 소비 패턴을 분석하기 위해 요금을 기록해주세요.</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with ledger_col:
        ...
        st.markdown("<div style='padding-bottom: 8px; border-bottom: 1px solid #E2E8F0; margin-bottom: 12px;'><span style='font-size:13.5px; font-weight:700; color:#1E293B;'>💰 요금 입력 (만원)</span></div>", unsafe_allow_html=True)
    ...
    with col_l:
        st.button("← 이전 단계", on_click=go_prev, use_container_width=True, key="btn_prev_step2")
    with col_r:
        st.button("다음 단계 (성능 평가) →", on_click=go_next, use_container_width=True, key="btn_next_step2")


# ──────────────────────────────────────────────────────────────
# STEP 3: 우리 건물 성능 정량 평가 (1~5단계)
# ──────────────────────────────────────────────────────────────
def render_step3():
    st.markdown("""
    <div class="hero">
...
def render_step4():
    ...
    <div class="hero">
        ...
        <div class="hero-sub">{st.session_state.region}</div>
    </div>
    """, unsafe_allow_html=True)
...
def render_step6():
    ...
    if dx:
        ...
        if grade_val in ["A+", "A", "B"]:
            grade_bg = "#059669" # 안전한 초록
            grade_shadow = "rgba(5,150,105,0.2)"
        elif grade_val == "C":
            grade_bg = "#F59E0B" # 주의 노랑/주황
            grade_shadow = "rgba(245,158,11,0.2)"
        else: # D, E, F
            grade_bg = "#DC2626" # 위험 빨강
            grade_shadow = "rgba(220,38,38,0.2)"
...
<div style="font-size:20px;color:#0F172A;font-weight:800;text-align:center;margin-bottom:20px;letter-spacing:-0.5px;display:flex;align-items:center;justify-content:center;gap:8px;">에너지 등급 <span style="color:#FFFFFF;background:{grade_bg};font-size:24px;font-weight:900;padding:4px 14px;border-radius:10px;box-shadow:0 2px 8px {grade_shadow};">{grade_val}</span></div>
